...
router.get('/pedidosComercialesJeronimoN8N_completa', async (req, res) => {
  try {
    const response = await fetch('http://192.168.1.81:3001/api/pedidosComercialesJeronimoN8N_completa');
    const data = await response.json();
    console.log('Datos recibidos ACCESS pedidosComerciales');
    res.json(data);
  } catch (err) {
