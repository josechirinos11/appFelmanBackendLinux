...
router.post("/n8n_dashboard", async (req, res) => {
  ...
  try {
    ...
    // 1) RESET (borrar todo lo anterior)
    await conn.execute("DELETE FROM terminales.n8n_pedidos_tipo_trabajo");
    await conn.execute("DELETE FROM terminales.n8n_pedidos_detalles");
    await conn.execute("DELETE FROM terminales.n8n_pedidos");
    ...
    if (rowsTipoTrabajo.length) {
      ...
      for (let i = 0; i < rowsTipoTrabajo.length; i += CHUNK) {
        ...
        await conn.execute(
          `
          INSERT INTO terminales.n8n_pedidos_tipo_trabajo
...
router.get("/alerta2", async (req, res) => {
  try {
    // 1. Obtener PEDIDOS desde ALMACÉN (poolAlmacen)
    ...
    const sqlPedidos = `
            ...
            WHERE p.Seccion = 'ALUMINIO'
        `;
    const [pedidosRows] = await poolAlmacen.query(sqlPedidos);
    ...
    if (pedidoKeys.length === 0) {
      // Caso raro: hay pedidos pero no generaron keys válidas
      return res.json([]);
    }

    // 3. Consultar DATOS DE FABRICACIÓN en TERMINALES (pool)
    ...
    const [terminalesRows] = await pool.query(sqlTerminales, params);
    ...
    resultado.sort((a, b) => {
      ...
      // Para ordenar correctamente por días positivos más pequeños primero, y negativos más pequeños (más urgentes):
      // Simplemente orden ASC numérico funciona. 
      // -5 (vencido) < 2 (urgente) < 10 (normal)
